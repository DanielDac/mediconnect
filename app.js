...
const sb = {
  ...
  async uploadImage(file) {
    try {
      ...
      console.log(
        "apikey usada (primeros 20 chars):",
      ...
      if (!r.ok) {
        ...
        if (r.status === 400)
          console.error(
            "  → 400: Solicitud inválida. Verifica Content-Type y que el body sea el archivo binario.",
          );
        if (r.status === 401)
          console.error(
            "  → 401: No autorizado. La apikey/anon key es incorrecta o no tiene acceso a Storage.",
          );
        if (r.status === 403)
          console.error(
            "  → 403: Prohibido. El bucket 'medicamentos' no tiene política RLS que permita INSERT al rol anon.",
          );
        if (r.status === 404)
          console.error(
            "  → 404: Bucket 'medicamentos' NO EXISTE en Supabase Storage. Créalo primero.",
          );
        console.error(
          "  Solución: Supabase → Storage → Buckets → medicamentos → Policies → New Policy → Allow INSERT for anon",
        );
        return null;
      }
  ...
  async testStorage() {
   ...
   // console.log("URL:", SUPABASE_URL);
    ...
    const r2 = await fetch(
      `${SUPABASE_URL}/storage/v1/object/list/medicamentos`,
      {
        method: "POST",
        headers: {
          apikey: SUPABASE_KEY,
    ...
    console.log(
      "Objetos en bucket 'medicamentos' (status",
      r2.status + "):",
      objs,
    );
...
getUser() {
  ...
  async register(nombre, email, password, rol) {
    try {
      ...
      // Si user es null → email duplicado
  ...
  async getDonations(filters = {}) {
    try {
      ...
      if (filters.receptor_id)
        query += `&receptor_id=eq.${filters.receptor_id}`;
    ...
    try {
      ...
      if (estado === "rechazado") {
        ...
        if (currentMed && currentMed.estado === "reservado") {
          console.log(
            "Detectado rechazo de reserva. Intentando restaurar stock...",
          );
          ...
          if (results && results.length > 0) {
            ...
            const newQty =
              parseInt(original.cantidad) + parseInt(currentMed.cantidad);
            console.log(
  ...
  async reserveDonation(id, requestedQuantity = null) {
    try {
      ...
      const toReserve = requestedQuantity
      ...
      if (toReserve === available) {
        ...
        // Reserva parcial:
        console.log(
          `Reservando parcial (${toReserve} de ${available}) de donación ${id}...`,
        );
  ...
  async processValidatorRequest(requestId, status, userId) {
    try {
      ...
      if (okReq && status === "aprobado") {
        const okUser = await sb.update("usuarios", userId, {
          rol: "validador",
        });
        return okUser;
      }
...
function startInactivityTimer() {
  ...
  // Guardar fecha de expiración en localStorage
...
// Al hacer login, iniciar temporizador
...
(function checkSessionExpiry() {
  ...
  if (expiry && Date.now() > parseInt(expiry)) {
    // La sesión expiró mientras la pestaña estaba cerrada
    mediConnect.logout();
    localStorage.clear();
    ...
    // Si hay usuario y no expiró, iniciar el temporizador
    startInactivityTimer();
  }
})();